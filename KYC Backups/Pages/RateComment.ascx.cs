...
public partial class RateComment : System.Web.UI.UserControl
{
    ...
    protected void rateUp_ServerClick(object sender, ImageClickEventArgs e)
    {
        string result = updateDB("up," + comment.InnerText);
        HttpCookie cookie;


        if (result == "true")
        { upv.InnerText = (int.Parse(upv.InnerText) + 1).ToString(); shownInfo.InnerText = "Thanks For Voting!";
        if (Request.Cookies["CommentVoted"] != null)
        {
            cookie = Request.Cookies["CommentVoted"];
            cookie[comment.InnerText] = "done";
        ...
        { shownInfo.InnerText = "Please try after sometime."; }
        else if (result == "done")
        { shownInfo.InnerText = "You have already voted!"; }
        shownInfo.Visible = true;
    }
    protected void rateDown_ServerClick(object sender, ImageClickEventArgs e)
    {
        string result = updateDB("down," + comment.InnerText);
        HttpCookie cookie;

        if (result == "true")
        { dnv.InnerText = (int.Parse(dnv.InnerText) + 1).ToString(); shownInfo.InnerText = "Thanks For Voting!";
        if (Request.Cookies["CommentVoted"] != null)
        {
            cookie = Request.Cookies["CommentVoted"];
            cookie[comment.InnerText] = "done";
